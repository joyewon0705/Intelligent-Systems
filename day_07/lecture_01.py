...
# - 머신러닝에 불필요한 정보를 확인한 후 제거할 수 있어야 함
#   ['PassengerId', 'Name', 'Ticket', 'Cabin'] 
#   (unique 값이 너무 많거나 데이터가 너무 적음)
...
# (Age와 Embarked의 데이터 개수가 많으므로 제거는 어려움)
...
# 반면 라벨 인코딩은 설명변수 X에는 잘 사용하지 않음...
# 값의 대소가 생기게 되고 무의미해지는 값이 발생할 수 있음
# EX) Sex * W
#     Embarked * W
...
# - 일반적으로 설명변수(X)에 포함된 문자열 데이터의 전처리에 활용됨
# - 메모리의 낭비가 심하므로 유일한 값의
#   케이스를 줄일 수 있는 방향으로 접근하는 것이 유의미하다.

# 문자열 전처리는 수치형 자료의 전처리와는
# 과정이 독립적이므로 데이터를 분할하는 것이 작성에 편리함
...
# 3. fit_transform 메소드 : 1번과 2번의 과정을 한 번에...
...
# 전처리된 데이터를 결합하여 설명변수 X를 생성
print(X_num.info())
print(X_obj_encoded.info())

# X_num에서 삭제된 인덱스가 있으므로 인덱스를 초기화해줌
...
# max_depth=None이므로 무조건 score는 1
